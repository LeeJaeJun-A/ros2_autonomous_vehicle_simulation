...
class MotionPlanningNode(Node):
    ...
    def timer_callback(self):
        ...
        # #3 좌회전 이후 s자 조향 #Added!!
        # elif self.parking_state == 'adjusting':
        #     elapsed = now - self.adjusting_start_time
        #     if elapsed < 4.0:
        #         self.steering_command = -1.0  # 좌조향
        #         self.left_speed_command = REVERSE_SPEED/2.5
        #         self.right_speed_command = REVERSE_SPEED
        #     elif elapsed < 8.0:
        #         self.steering_command = 1.0   # 우조향
        #         self.left_speed_command = REVERSE_SPEED
        #         self.right_speed_command = REVERSE_SPEED/2.5
        #     else:
        #         self.steering_command = 0.0
        #         self.parking_state = 'reversing'
        #         self.reversing_start_time = now
        #         self.get_logger().info("Orientation adjusted. Starting to reverse.")
        #         self.received_start_angles = []
        #         self.received_end_angles = []
        #         self.adjusting_start_time = None  # 초기화
        #         self.left_speed_command = REVERSE_SPEED
        #         self.right_speed_command = REVERSE_SPEED
        ...
        #3 후진 및 조향 각도 계산 (후방 물체 감지 후)
            ...
            #     # 중심 각도 계산
            #     center_angle = (max_end_angle + min_start_angle) / 2.0 
                
            #     # 중심각에서 180도(정면)까지의 편차를 -1.0 ~ 1.0 범위로 정규화
            #     error_angle = center_angle - 180.0
            #     self.steering_command = np.clip(error_angle / 20.0, -1.0, 1.0)

            #     self.get_logger().info(f"Reversing: center_angle={center_angle:.2f}, steering_command={self.steering_command:.2f}")
            # else:
            #     self.steering_command = 0.0
                # self.get_logger().info("Reversing straight initially.")
            ...
            #     center_angle = (min_end_angle + max_start_angle) / 2.0
            #     error_angle = center_angle - 180.0

            #     # ✅ 추가: 위치 offset 계산
            #     offset_angle = (max_start_angle - min_end_angle) / 2.0

            #     # ✅ 개선된 조향 계산: 각도 + offset 반영
            #     combined_error = error_angle + offset_angle * 1.5  # 1.5는 튜닝 가능한 계수

            #     # ✅ 정규화해서 조향 커맨드 만들기
            #     self.steering_command = np.clip(combined_error / 30.0, -1.0, 1.0)
                
            #     self.get_logger().info(
            #         f"Reversing: center={center_angle:.2f}, offset={offset_angle:.2f}, steering={self.steering_command:.2f}"
            #     )
            # else:
            #     self.steering_command = 0.0
            #     self.get_logger().info("Reversing straight initially.")
