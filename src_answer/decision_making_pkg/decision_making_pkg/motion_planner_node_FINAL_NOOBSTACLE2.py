...
#--------------------------------------횡단보도 차선 변경 로직.
        if self.traffic_light_bbox is not None:
            ...
            if self.lane_control == False:
                if h > 30 and not self.traffic_light_flag_toggled and self.traffic_cnt >= 3:
            ...
            elif self.lane_control == True:
                if h > 50 and not self.traffic_light_flag_toggled and self.traffic_cnt >= 3:
                    self.lane_control = not self.lane_control
                    self.traffic_light_flag_toggled = True
                    self.traffic_light_toggle_time = self.get_clock().now().nanoseconds
        ...
        if self.lane_control is False: #2차선.
            if self.path_data is None:
                ...
                if abs_target_slope > 55: 
                    self.steering_command = int(target_slope/9.5)
            ...
            if self.path_data is None:
                ...
                if self.traffic_light :#or self.get_clock().now().nanoseconds - self.traffic_light_toggle_time <= 2 * 1000000000:
                    if not self.traffic_light_bbox:
                        self.left_speed_command = SPD  # 예시 속도 값 (255가 최대 속도)
                        self.right_speed_command = SPD   # 예시 속도 값 (255가 최대 속도)
                        self.cnt += 1    
                    else:
                ...
                elif target_slope < -72:
                    self.left_speed_command = CSPD
                    self.right_speed_command = CSPD+50
