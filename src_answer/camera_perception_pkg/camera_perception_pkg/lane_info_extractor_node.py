...
################################
# 추가: std_msgs.msg에서 Bool 메시지를 임포트합니다.
################################
...
class Yolov8InfoExtractor(Node):
    def __init__(self):
        ...
        ################################
        # 추가: flag 토픽에 대한 구독자 추가 (Bool 메시지)
        ################################
        self.flag_subscriber = self.create_subscription(Bool, "flag_topic", self.flag_callback, self.qos_profile)
        # 기본 flag 값 False => lane2 사용
        self.flag = False
        ###############################

    ################################
    # 추가: flag 업데이트를 위한 콜백 함수
    ################################
    def flag_callback(self, msg: Bool):
        self.flag = msg.data
        self.get_logger().info(f"Flag updated: {self.flag}")
    ###############################
    ...
    def yolov8_detections_callback(self, detection_msg: DetectionArray):
        ...
        ################################
        # 수정: flag 값에 따라 lane 이름 결정 (False: lane2, True: lane1)
        ################################
        lane_class = 'lane1' if self.flag else 'lane2'
        lane_edge_image = CPFL.draw_edges(detection_msg, cls_name=lane_class, color=255)
        ###############################
        ...
        lane2_bird_image = CPFL.bird_convert(lane_edge_image, srcmat=src_mat, dstmat=dst_mat)
        roi_image = CPFL.roi_rectangle_below(lane2_bird_image, cutting_idx=300)     #몇 픽셀정도를 자를것인지에 대한 표기. 480*640에서 300개 자른것.

        if self.show_image:
            cv2.imshow('lane2_edge_image', lane_edge_image)
        ...
        grad = CPFL.dominant_gradient(roi_image, theta_limit=70)  #기울기 추출해주는 코드. 하이퍼 파라미터로 차 앞대가리는 인식 안하게 각도 제한치 설정해둔것.
        ...
        for target_point_y in range(5, 155, 30):  # 예시로 5에서 155까지 50씩 증가
            target_point_x = CPFL.get_lane_center(roi_image, detection_height=target_point_y, 
                                                detection_thickness=10, road_gradient=grad, lane_width=300) #thickness는 라인 끊길때 대비해 여러 픽셀의 두께로 그음.
